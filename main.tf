// module for terraform versions
module  "a_versions" {
  source = "./module/a_versions"
...
resource "aws_vpc" "main_vpc" {
  ...
  // We are tagging the VPC with the name "main_vpc"
  tags = {
    Name = "main_vpc"
  }
}

// Create an internet gateway named "main_igw"
// and attach it to the "main_vpc" VPC
resource "aws_internet_gateway" "main_igw" {
  // Here we are attaching the IGW to the 
  // main_vpc VPC
  vpc_id = aws_vpc.main_vpc.id

  // We are tagging the IGW with the name main_igw
  tags = {
    Name = "main_igw"
  }
}
...
resource "aws_subnet" "main_public_subnet" {
  ...
  // Put the subnet into the "main_vpc" VPC
  vpc_id            = aws_vpc.main_vpc.id
  ...
  // We are tagging the subnet with a name of "main_public_subnet_" and
...
resource "aws_subnet" "main_private_subnet" {
  ...
  // We are tagging the subnet with a name of "main_private_subnet_" and
  // suffixed with the count
  tags = {
...
// Create a public route table named "main_public_rt"
resource "aws_route_table" "main_public_rt" {
  // Put the route table in the "main_vpc" VPC
  vpc_id = aws_vpc.main_vpc.id
...
resource "aws_route_table_association" "public" {
  ...
  // "main_public_rt" from above
  route_table_id = aws_route_table.main_public_rt.id
  
  // This is the subnet ID. Since the "main_public_subnet" is a 
...
// Create a private route table named "main_private_rt"
...
resource "aws_security_group" "main_web_sg" {
  ...
  // Here we are tagging the SG with the name "main_web_sg"
  tags = {
    Name = "main_web_sg"
  }
}

// Create a security group for the RDS instances called "main_db_sg"
resource "aws_security_group" "main_db_sg" {
  // Basic details like the name and description of the SG
  name        = "main_db_sg"
  description = "Security group for main databases"
  // We want the SG to be in the "main_vpc" VPC
  vpc_id      = aws_vpc.main_vpc.id
...
resource "aws_db_instance" "main_database" {
  ...
  // settings.database.db_name variable, which is set to "main"
  ...
  // This is the DB subnet group "main_db_subnet_group"
...
// Create a key pair named "main_kp"
...
resource "aws_instance" "main_web" {
  ...
  // The subnet ID for the EC2 instance. Since "main_public_subnet" is a list
  ...
  // "main_public_subnet" and putting the EC2 instance in there
  subnet_id              = aws_subnet.main_public_subnet[count.index].id
  
  // The key pair to connect to the EC2 instance. We are using the "main_kp" key 
  ...
  // We are tagging the EC2 instance with the name "main_db_" followed by
  // the count index
  tags = {
...
// Create an Elastic IP named "main_web_eip" for each
// EC2 instance
resource "aws_eip" "main_web_eip" {
  ...
  count    = var.settings.web_app.count
	...
	// The EC2 instance. Since main_web is a list of 
